...
class PermissionAPITestCase(BaseAPITestCase):
    ...
    def test_role_create_view_with_permission(self):
        ...
        role = Role.objects.get(name=TEST_ROLE_2_LABEL)
        self.assertEqual(response.data, {'name': role.name, 'id': role.pk})
        self.assertEqual(Role.objects.count(), 2)
        self.assertEqual(role.name, TEST_ROLE_2_LABEL)
    ...
    def test_role_create_complex_view_no_permission(self):
        ...
        self.assertEqual(Role.objects.count(), 1)
        self.assertEqual(
            list(Role.objects.values_list('name', flat=True)),
            [TEST_ROLE_LABEL]
        )
    ...
    def test_role_edit_via_patch_no_access(self):
        ...
        self.assertEqual(self.role.name, TEST_ROLE_LABEL)
    ...
    def test_role_edit_via_patch_with_access(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.role.refresh_from_db()
        self.assertEqual(self.role.name, TEST_ROLE_LABEL_EDITED)

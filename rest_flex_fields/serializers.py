...
class FlexFieldsSerializerMixin(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.diable_query_params = True
        passed = {
            'expand': kwargs.pop('expand', None),
            'fields': kwargs.pop('fields', None),

        }
        passedexpandable_fields = kwargs.pop('expandable_fields', {})
        super(FlexFieldsSerializerMixin, self).__init__(*args, **kwargs)
        if getattr(self, 'Meta', None) and getattr(self.Meta, 'expandable_fields', None):
            self.expandable_fields = self.Meta.expandable_fields

        self.expandable_fields.update(passedexpandable_fields)
    ...
    def _get_fields_input(self, passed_settings):
        ...
        if not self._can_access_request:
            return None
        if not self.diable_query_params:

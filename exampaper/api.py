...
from .serializers import ExamPaPerSerializer, ZipfileSerializer, addquestionfunc, ExamPaPerCreateSerializer, ExamPaPerMembersSerializer
...
questiondict = openapi.Parameter('questiondict',
                                 in_=openapi.IN_QUERY,
                                 description='返回问题是否归类',
                                 type=openapi.TYPE_BOOLEAN)


@method_decorator(name='retrieve', decorator=swagger_auto_schema(manual_parameters=[questiondict]))
class ExamPaPerViewSet(viewsets.ModelViewSet):
    ...
    def get_serializer(self, *args, **kwargs):
        if self.action == 'retrieve':
            # kwargs.update(expand='multichoices,judgements,singlechoices')
    ...
    def get_queryset(self):
        ...
        try:
            if exclude == 'true':
                ...
                return User.objects.filter(role=1).exclude(exampapers__id=paperid)
            else:
                paper = ExamPaPer.objects.get(id=paperid)
